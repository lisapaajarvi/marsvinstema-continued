...
class CartProvider extends Component<{}, State> {
    ...
    addProductToCart = (product: Product) => {
        const cartItem = this.state.cart.find( item=> item._id === product._id)
        if (cartItem) {
            cartItem.quantity ++;
    ...
    removeProductFromCart = (product: Product) => {
        const updatedCart = this.state.cart.filter(item => item._id !== product._id);
        this.setState({ cart: updatedCart });
        this.updateCartInLocalStorage(updatedCart)
    }

    increaseQuantity = (product: string) => {
        const cartItem = this.state.cart.find( item=> item._id === product);
        if (cartItem) {
            cartItem.quantity ++;
            this.setState({ cart: this.state.cart }); 
            this.updateCartInLocalStorage(this.state.cart)
    ...
    decreaseQuantity = (product: string) => {
        ...
        if (cartItem && cartItem.quantity > 1) {
            ...
            const updatedCart = this.state.cart.filter(item => item._id !== product);
            this.setState({ cart: updatedCart });
            this.updateCartInLocalStorage(updatedCart);
        } 
    }
